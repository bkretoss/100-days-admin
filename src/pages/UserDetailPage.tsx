...
const UserDetail: React.FC = () => {
  ...
  const fallbackAvatarUrl = `https://api.dicebear.com/7.x/avataaars/svg?seed=${encodeURIComponent(displayName)}&backgroundColor=1a1d24`;
        ...
        <div className="space-y-6">
          ...
          <div className="glass-card p-8 flex flex-col items-center text-center relative overflow-hidden">
            ...
            <div className="size-24 rounded-full bg-dark-800 border-4 border-white/5 flex items-center justify-center text-3xl font-black text-white mb-4 shadow-2xl shadow-electric-blue/10 overflow-hidden">
              {showRemote ? (
                <img src={remoteUrl!} alt={displayName} className="w-full h-full object-cover" onError={() => setImgFailed(true)} />
              ) : remoteUrl && imgFailed ? (
                <span className="text-2xl leading-none">{initials}</span>
              ) : (
                <img src={fallbackAvatarUrl} alt="" className="w-full h-full object-cover" />
              )}
            </div>
          ...
          <div className="glass-card p-6 space-y-6">
            ...
            <div className="space-y-4">
              {[
                ...
                { label: "Start Date", value: <span className="text-sm font-bold text-white">{user ? formatDate(user.startDate) : "—"}</span> },
                { label: "End Date", value: <span className="text-sm font-bold text-white">{user ? formatDate(user.endDate) : "—"}</span> },
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-white/5 text-gray-400 text-[10px] uppercase tracking-wider font-bold">
                      {["Plan", "Status", "Start Date", "End Date", "Device Type"].map((h) => <th key={h} className="px-4 py-3">{h}</th>)}
                    </tr>
                  </thead>
